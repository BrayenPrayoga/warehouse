...
class MasukRakController extends Controller
{
    ...
    public function index(){
        ...
        $data['no'] = 1;
        $data['data'] = Barang::join('tabel_proses_masuk','id_barang','tabel_barang.id')->where('status', 1)->whereNotNull('id_rak')->orderBy('tabel_barang.id','ASC')->get();
        ...
        if($check_rak){
            if($check_barang){
                if($check_barang->status == 0){
                    ...
                    $barang = Barang::with('RelasiKategori','RelasiRak')->join('tabel_proses_masuk','id_barang','tabel_barang.id')->where('status', 1)->whereNotNull('id_rak')->orderBy('tabel_barang.id','ASC')->get();
